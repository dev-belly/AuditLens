...
[![Tests: 412](https://img.shields.io/badge/tests-412%20passing-brightgreen.svg)](#testing)
...
| Test suite | **412 tests**, all passing — including in-process render tests for all six dashboard pages |
...
The dashboard reads the SQLite warehouse. The ground-truth columns do travel with it —
they are in `transactions` for the same reason they are in the parquet, so the model
evaluation can be reproduced — but no page can render them. The grid builders list their
columns explicitly rather than slicing `frame.columns`, so a column added to the pipeline
cannot reach an auditor's screen by default. `tests/test_dashboard.py` renders all six
pages in-process with `AppTest` and fails if any page raises, produces no content, or
emits a ground-truth column.
...
<tr>
<td width="50%"><img src="docs/screenshots/03_audit_rules.png" alt="Audit Rules page"><br>
<sub><b>Audit Rules</b> — each of the nine procedures with its threshold, rationale and measured precision/recall.</sub></td>
<td width="50%"><img src="docs/screenshots/04_benford_analysis.png" alt="Benford Analysis page"><br>
<sub><b>Benford Analysis</b> — observed vs expected digits, MAD and χ², with the "not a fraud test" caveat stated on the page itself.</sub></td>
</tr>
<tr>
<td width="50%"><img src="docs/screenshots/05_machine_learning.png" alt="Machine Learning page"><br>
<sub><b>Machine Learning</b> — confusion matrix and the precision/recall-vs-review-budget curve, with the benchmark caveat stated up front.</sub></td>
<td width="50%"><img src="docs/screenshots/06_vendor_risk.png" alt="Vendor Risk page"><br>
<sub><b>Vendor Risk</b> — counterparty ranking, shared bank accounts, dormancy reactivation and spend concentration.</sub></td>
</tr>
</table>

These are real captures, not mockups: `tools/capture_screenshots.py` drives headless
Chromium over the DevTools Protocol and waits for each page to finish rendering before
capturing. `make screenshots` regenerates them.
...
├── tests/                       # 412 tests, incl. cross-process reproducibility
├── docs/
...
│   └── screenshots/             # 6 committed dashboard captures + how to regenerate them
├── outputs/
...
├── .github/workflows/ci.yml     # pipeline + 412 tests + a reproducibility check, on 3.11 and 3.12
└── Makefile
```
...
cell and embeds what it produced** — stdout, the formatted tables and the matplotlib
figures — before writing the file. A notebook that does not run cannot be committed, and
a cell that silently renders empty cannot be committed either: `tests/test_notebooks.py`
fails if any cell that prints or plots has no output.

That test also enforces *stability*. Two things used to make the committed notebooks
differ on every build even when every number matched: pandas stamps each `Styler` render
with a fresh `uuid4` table id, and the pipeline's loggers write a wall-clock timestamp
into whatever `sys.stdout` was live when they were first built. Both are normalised by the
builder, so two consecutive builds are byte-identical.
...
412 tests, all passing. `make test` runs the lot; `make test-fast` skips the dashboard
render suite.
...
| `test_readme_claims.py` | 25 | Every headline figure, all nine rules' flagged/precision/recall, the Benford table, the risk-band counts and values, the component weights, and the structural counts the prose quotes ("9 stages", "20 features", "9 patterns", and how many mistakes the methodology records) — checked against `outputs/reports/` and against the code |
