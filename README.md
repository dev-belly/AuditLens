...
[![Tests: 388](https://img.shields.io/badge/tests-388%20passing-brightgreen.svg)](#testing)
...
| Test suite | **388 tests**, including dashboard render and warehouse integrity checks |
...
├── tests/                       # 388 tests, incl. cross-process reproducibility
├── docs/
...
├── .github/workflows/ci.yml     # pipeline + 388 tests + a reproducibility check, on 3.11 and 3.12
└── Makefile
```
...
python -m pytest tests/ -q     # 388 tests
```
...
388 tests. `make test` runs the lot; `make verify` first regenerates the ledger,
...
| `test_warehouse_contract.py` | 6 | Rejects partial SQLite builds, duplicate keys and orphaned alerts; preserves a usable warehouse when rebuilding fails; checks analyst queries and persisted row counts |
