"""The notebook cell definitions: what each of the three notebooks contains.

Kept apart from the build machinery in :mod:`tools.build_notebooks`, because the two
change for entirely different reasons. This file changes when the *analysis* changes -
a new chart, a reworded explanation - and it reads top to bottom like prose. The
machinery changes when nbformat, pandas or matplotlib change how cell output is
captured.

Together they had grown past a thousand lines, at which point anyone looking for the
output-capture logic had to scroll past eight hundred lines of cell text to reach it.

This extraction is a pure move: the same cells, in the same order, with the same
``PRELUDE``. ``tests/test_notebooks.py`` and a byte-for-byte notebook diff pin it.
"""
